    ...
    if (result.isConfirmed) {
      $.ajax({
        ...
        error: function (xhr, er) {
          swal.fire(
            "Atenção!",
            "Ocorreu um erro ao retornar os dados!",
            "error"
          );
        },
      });
...
function selectUsers() {
  $.ajax({
    ...
    success: function (result) {
      $(
        "#slEtapResponsavel, #slEtapResponsavel, #slRespAtividade, #slRespTarefa"
      ).prop("disabled", false);
      $(
        "#slEtapResponsavel, #slEtapResponsavel, #slRespAtividade, #slRespTarefa"
      ...
      $(
        "#slEtapResponsavel, #slEtapResponsavel, #slRespAtividade, #slRespTarefa"
      ).append('<option value=""> Responsável </option>');

      var jsonData1 = JSON.stringify(result);
      $.each(JSON.parse(jsonData1), function (idx, obj) {
        $(
          "#slResponsavel, #slRespProjeto, #slEtapResponsavel, #slRespAtividade, #slRespTarefa"
        )
          .append(
            '<option value="' + obj.id_users + '">' + obj.nome + "</option>"
          )
          .selectpicker("refresh");
      });
      swal.fire({
...
function selectDepto() {
  $.ajax({
    ...
    success: function (result) {
      ...
      $("#slDepProjeto,#slAtivDepto").append(
        '<option value="">Departamentos</option>'
      );
      var jsonData1 = JSON.stringify(result);
      $.each(JSON.parse(jsonData1), function (idx, obj) {
        $("#slDepProjeto,#slAtivDepto")
          .append(
            '<option value="' +
              obj.id_departamento +
              '">' +
...
function selectProjetos(value, opt) {
  $.ajax({
    ...
    success: function (result) {
      ...
      $("#slEtapProjeto, #slAtivProjeto").append(
        '<option value="">Projetos</option>'
      );
      var jsonData1 = JSON.stringify(result);
      $.each(JSON.parse(jsonData1), function (idx, obj) {
        $("#slEtapProjeto, #slAtivProjeto")
          .append(
            '<option value="' +
              obj.id_projeto +
              '">' +
      ...
      if (
        typeof $("#txtIdAtividade").val() != "undefined" &&
        $("#txtIdAtividade").val() != ""
      ) {
...
function selectEtapas(value, etapa) {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      $("#slAtivEtapas,#slAtivEtapas").append(
        '<option value="">Etapas</option>'
      );
      var jsonData1 = JSON.stringify(result);
      $.each(JSON.parse(jsonData1), function (idx, obj) {
        $("#slAtivEtapas,#slAtivEtapas")
          .append(
            '<option value="' +
              obj.id_etapa +
              '">' +
...
function viewAnexo(value) {
  if (value != "" && value != null) {
    return (
      '<buttom class="btn btn-outline-primary btn-sm" onclick="modalAnexo(\'' +
      value +
      '\');"><i class="fa-regular fa-images"></i></button'
    );
  }
}
...
function sequencia(value) {
  return (
    '<button style="cursor: auto;" class="btn btn-sm btn-outline-dark px-3">' +
    value +
    "</button>"
  );
}
...
function situacao(value) {
  if (value == "A") {
    return `<button class="btn btn-sm btn-outline-dark btn-block" style="cursor: auto;" ${
      myarr[idc] == "etapas"
        ? ""
        : 'data-target="#modalAltSituacao" data-toggle="modal"'
    }>Aguardando</button>`;
  } else if (value == "P") {
    return `<button class="btn btn-sm btn-outline-danger btn-block" style="cursor: auto;" ${
      myarr[idc] == "etapas"
        ? ""
        : 'data-target="#modalAltSituacao" data-toggle="modal"'
    }>Pendente</button>`;
  } else if (value == "E") {
    return `<button class="btn btn-sm btn-outline-warning btn-block" style="cursor: auto;" ${
      myarr[idc] == "etapas"
        ? ""
        : 'data-target="#modalAltSituacao" data-toggle="modal"'
    }>Executando</button>`;
  } else if (value == "C") {
    return `<button class="btn btn-sm btn-outline-success btn-block" style="cursor: auto;" ${
      myarr[idc] == "etapas"
        ? ""
        : 'data-target="#modalAltSituacao" data-toggle="modal"'
    }>Concluida</button>`;
  } else if (value == "I") {
    return `<button class="btn btn-sm btn-outline-primary btn-block" style="cursor: auto;" ${
      myarr[idc] == "etapas"
        ? ""
        : 'data-target="#modalAltSituacao" data-toggle="modal"'
    }>Iniciada</button>`;
  } else if (value == "R") {
    return `<button class="btn btn-sm btn-outline-secondary btn-block" style="cursor: auto;">Revisada</button>`;
  }
}
...
function altsituacao(id_atividade) {
  ...
  if($('#slAltSituacao').val() == ''){
    swal.fire('Atenção!', 'Selecione um status para a atividade.', 'info');
    return;
  }
  ...
  $.ajax({
    ...
    error: function () {
      swal.fire(
        "Atenção!",
        "Ocorreu um erro ao tentar registrar os dados!",
        "error"
      );
    },
    beforeSend: function () {
    ...
    success: function (data) {
      ...
      if (data.code == 0) {
        ...
        $('#slAltSituacao').selectpicker('val', '')
        $("#tableAtividades").bootstrapTable("refresh");
                                        ...
                                        <div class="col-12 p-0">
                                            <div class="col-12">
                                                <table class = "table table-bordered">
                                                    <tbody>`;
        var arrayProjetos = dashboardProjetos(
          departamento.descrDepartamento,
          result
        );
            ...
            `                          <tr>
                                                            <td class = "col-2" >
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col-3">
                                                                        <div onclick='altProjeto(` +
            JSON.stringify(Projetos) +
            `);'<i class="fa-solid fa-list fa-2x" style="color: #337566; cursor:pointer" data-tooltip="EDITAR PROJETO"></i></div>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-12 text-center pt-2">
                                                                        ...
                                                                        <button class="btn btn-outline-danger btn-sm" onclick='altEtapasDash(` +
            JSON.stringify(Projetos) +
            `);'>Cadastrar Etapas</button>
                                                                    </div>
                                                                </div>
                                                                </td> 
                                                            <td class = "col-10">
                                                                    ...
                                                                    <div class="container-fluid p-1">
                                                                        <div class="row">
                                                                            <div class="col-md-3 pl-2 pr-0">
                                                                                        ...
                                                                                        `;
          var arrayEtapas = dashboardEtapas(
            departamento.descrDepartamento,
            Projetos.descrPropjeto,
            result
          );
                                                                                        ...
                                                                                        <div class="alert bg-dark p-1 mb-1">
                                                                                            <div class="row px-2">
                                                                                                ...
                                                                                                <div class="col-4 text-right">
                                                                                                    <a href="#"  onclick='cadAtividadeDash(` +
                JSON.stringify(etapas) +
                `);'><i data-tooltip="Adicionar atividade" class="fa-brands fa-creative-commons-nd" style="color: #FFF;"></i></a>
                                                                                                    <a href="#" onclick='altEtapas(` +
                JSON.stringify(etapas) +
                `);'><i data-tooltip="EDITAR ETAPA" class="fa-solid fa-list" style="color: #FFF;"></i></a>
                                                                                                </div>
                                                                                            </div>
                                                                                                `;
              var arrayAtividade = dashboardAtividades(
                departamento.descrDepartamento,
                Projetos.descrPropjeto,
                etapas.id_etapa,
                result
                  ...
                  html +=
                    `                                                 
                                                                                            <div class="alert p-1 bg-light  my-1" role="alert">
                                                                                                <div class="row">
                                                                                                    <div class="col-6">
                                                                                                        <h6 class="alert-heading p-0  m-0" style="font-size: 12px;">${
                                                                                                          atividades.nomeAtividade
                                                                                                        }</h6>
                                                                                                    </div>
                                                                                                    <div class="col-6 text-right">
                                                                                                        ${situation(
                                                                                                          atividades.sitAtividade,
                                                                                                          result
                                                                                                        )}
                                                                                                        <buttom  ` +
                    (atividades.anexoAtividade != ""
                      ? `onclick="modalAnexo('${atividades.anexoAtividade}');" data-tooltip="VISUALIZAR ANEXO" style="color: #000; cursor: pointer;"`
                      : `style="cursor: not-allowed; color: #b0b0b0;" data-tooltip="SEM ANEXO"`) +
                    `><i class="fa-regular fa-images" class="bg-success"></i></buttom>
                                                                                                        <a href="#" onclick='altAtividade(` +
                    JSON.stringify(atividades) +
                    `);'<i data-tooltip="EDITAR ATIVIDADE" class="fa-solid fa-list" style="color: #ea3434;"></i></a>
                                                                                                    </div>
                                                                                                </div>
                                                                                            </div>
                            `;
                ...
                ` 
                                                                                            ...
                                                                                            <div class="row px-2">
                                                                                                ...
                                                                                                <div class="col-4 text-right">
                                                                                                    <a href="#"  onclick='cadAtividadeDash(` +
                JSON.stringify(etapas) +
                `);'><i data-tooltip="Adicionar atividade" class="fa-brands fa-creative-commons-nd" style="color: #000;"></i></a>
                                                                                                    <a href="#" onclick='altEtapas(` +
                JSON.stringify(etapas) +
                `);'<i data-tooltip="EDITAR ETAPA" class="fa-solid fa-list" style="color: #000;"></i></a>
            ...
            `                                                      
                                                                                        <div class="alert p-1 mb-1 bg-dark">
                                                                                            <div class="row px-2">
                                                                                                ...
                                                                                                <div class="col-4 text-right">
                                                                                                    <a onclick='$("#divAnexo").removeClass("d-none"); cadTarefaDash(` +
            JSON.stringify(dados) +
            `, "tarefa");'><i data-tooltip="ADICIONAR TAREFA" class="fa-brands fa-creative-commons-nd" style="color: #FFF;"></i></a>
                                                                                                    <a onclick='$("#divAnexo").addClass("d-none"); altCabec(` +
            JSON.stringify(dados) +
            `, "cabec" ); flagEditCabec = true;'><i data-tooltip="EDITAR CABEÇALHO" class="fa-solid fa-list" style="color: #FFF;"></i></a>
                                                                                                </div>
                                                                                            </div>
                                                                                                `;
                                                                                            ...
                                                                                            <div class="alert p-1 bg-light  my-1" role="alert">
                                                                                                <div class="row">
                                                                                                    ...
                                                                                                    <div class="col-6 text-right">
                                                                                                        ...
                                                                                                        <buttom><i class="fa-regular fa-images"  ` +
                (tarefas.anexo != ""
                  ? `onclick="modalAnexo('${tarefas.anexo}');" data-tooltip="VISUALIZAR ANEXO" style="color: #000;"`
                  : `style="cursor: not-allowed; color: #b0b0b0;" data-tooltip="SEM ANEXO"`) +
                `  class="bg-success"></i></buttom>
                                                                                                        <a  onclick='$("#divAnexo").removeClass("d-none"); altCabec(` +
                JSON.stringify(tarefas) +
                `, "tarefa");'><i class="fa-solid fa-list" data-tooltip="EDITAR TAREFA" style="color: #ea3434;"></i></a>
                                                                                                    </div>
                                                                                                </div>
                                                                                            </div>
                            `;
            ...
            ` 
                                                                                            ...
                                                                                            <div class="row px-2">
                                                                                                ...
                                                                                                <div class="col-4 text-right">
                                                                                                    <a   onclick='$("#divAnexo").removeClass("d-none"); cadTarefaDash(` +
            JSON.stringify(dados) +
            `, "tarefa");'><i data-tooltip="ADICIONAR TAREFA" class="fa-brands fa-creative-commons-nd" style="color: #000;"></i></a>
                                                                                                    <a  data-tooltip="EDITAR CABEÇALHO" onclick='$("#divAnexo").addClass("d-none");altCabec(` +
            JSON.stringify(dados) +
            `, "cabec");  flagEditCabec = true;'<i class="fa-solid fa-list" style="color: #000;"></i></a>
                                                                                                    ...
                                                                                                    <div class="alert p-1 bg-light  my-1" role="alert">
                                                                                                        <div class="row">
                                                                                                            ...
                                                                                                            <div class="col-6 text-right">
                                                                                                                ...
                                                                                                                <buttom><i class="fa-regular fa-images" ` +
                (tarefas.anexo != ""
                  ? `onclick="modalAnexo('${tarefas.anexo}');" style="color: #000;" data-tooltip="VISUALIZAR ANEXO"`
                  : `style="cursor: not-allowed; color: #b0b0b0;" data-tooltip="SEM ANEXO"`) +
                ` class="bg-success"></i></buttom>
                                                                                                                <a  onclick='$("#divAnexo").removeClass("d-none"); altCabec(` +
                JSON.stringify(tarefas) +
        ...
        if (dados.situacao == "C") {
          html +=
            ...
            `, "tarefa");'><i data-tooltip="ADICIONAR TAREFA" class="fa-brands fa-creative-commons-nd" style="color: #FFF;"></i></a>
                                                                                                        <a  data-tooltip="EDITAR CABEÇALHO" onclick='$("#divAnexo").addClass("d-none"); altCabec(` +
            JSON.stringify(dados) +
            `, "cabec"); flagEditCabec = true;'<i class="fa-solid fa-list" style="color: #FFF;"></i></a>
                                                                                                    </div>
                                                                                                </div>
                                                                                            `;
          // var arrayAtividade = dashboardAtividades(departamento.descrDepartamento, Projetos.descrPropjeto, etapas.id_etapa, result);
...
function dashboardDeptos(value) {
  ...
  $.each(JSON.parse(jsonDepto), function (idx, obj) {
    if (countDepto.indexOf(obj.descrDepartamento) > -1) {
    } else {
      deptos.push(obj);
...
function dashboardProjetos(depto, result) {
  ...
  $.each(JSON.parse(jsonProjetos), function (idx, obj) {
    if (countProjeto.indexOf(obj.descrPropjeto) > -1) {
    } else {
...
function dashboardEtapas(depto, projetos, result) {
  ...
  $.each(JSON.parse(jsonEtapas), function (idx, obj) {
    if (etapas.indexOf(obj.descrEtapa) > -1) {
    } else {
      if (obj.descrDepartamento == depto) {
...
function altAtividade(value) {
  ...
  if (typeof value.dtEntregaAtividade == "string") {
    $("#txtDataFimAtividade").val(
      value.dtEntregaAtividade.split("/").reverse().join("-")
    );
  }
...
function cadAtividadeDash(value) {
  ...
  $("#slAtivEtapas")
    .append(
      '<option value="' + value.id_etapa + '">' + value.nomeEtapa + "</option>"
    )
    .selectpicker("refresh");
  $("#ModalAtividades").modal("show");
}
